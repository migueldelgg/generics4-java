package application;

import entities.Circle;
import entities.Rectangle;
import entities.Shape;

import java.util.ArrayList;
import java.util.List;

public class Program {
    public static void main(String[] args) {

        List<Shape> myShapes = new ArrayList<>();
        myShapes.add(new Rectangle(3.0, 2.0));
        myShapes.add(new Circle(2.0));

        List<Circle> circleList = new ArrayList<>();
        circleList.add(new Circle(2.0));
        circleList.add(new Circle(3.0));

        System.out.println("Total area: " + totalArea(circleList));
    }

    public static double totalArea(List<? extends Shape> list) { // pode ser de Shape ou de qualquer subtipo de Shape
        double sum = 0.0;
        for (Shape s : list) {
            sum += s.area();
        }
        return sum;

        /* Nota 1: soluções impróprias:
            public double totalArea(List<Shape> list)
            public double totalArea(List<?> list)

        Nota 2: não conseguiremos adicionar elementos na lista do método
         */

    }
}
